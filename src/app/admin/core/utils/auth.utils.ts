 ...
 * Basado en Auth0 Angular-JWT helper service
 */
...
export class AuthUtils {
   ...
   * @param offsetSeconds - Segundos de offset para considerar expiración
   * @returns true si el token está expirado
   */
  static isTokenExpired(token: string, offsetSeconds?: number): boolean {
    // Retornar si no hay token
    if (!token || token === '') {
      return true;
    }
  ...
  private static _decodeToken(token: string): any {
    ...
    // Decodificar el token usando el decodificador Base64
  ...
  private static _getTokenExpirationDate(token: string): Date | null {
    ...
    // Retornar si el decodedToken no tiene un campo 'exp'
